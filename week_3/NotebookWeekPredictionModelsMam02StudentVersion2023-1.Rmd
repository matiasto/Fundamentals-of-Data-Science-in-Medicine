...
bc <- .....BreastCancer....  # dataset with complete cases
# => How many obseravtion (rows) are there now?
# How many cases did we remove?

# remove id column
bc <-  #=> Remove the "id" column from bc. Note that if you consider to use the "select" command in dplyr then this command may clash with the select command in the MASS library. Therefore use dplyr::select in that case.
...
...
train <-  ... # => Obrain train set consisting of 70% of the data
test <-  ... #=> Obtain test set consisting of the rest of observations

mitoses.train.lrm <- ... # => fit lrm model on the training set using only Mitoses, use x=T and y=T
mitoses.thickness.train.lrm <- ... # => fit lrm model on the training set using  Mitoses and Cl.thickness, use x=T and y=T
```

Now predict on the test set
```{r}
predicted.mitoses.test <- ... # => obtain the predicted probabilities of mitoses.train.lrm on the test set. Hint: use the "predict" function. Important note: make sure you use the right "type" in the command to get probabilities and not log odds.
predicted.mitoses.thickness.test <- ... # => obtain the predicted probabilities of mitoses.thickness.train.lrm on the test set.Check that they indeed are between 0 and 1 and have no negative numbers etc.
```
...
# For those without breast cancer the probabilities are concentrated below 0.4
# For those with breast cancer the probabilities are concentrated above 0.8
# For those without breast cancer the probabilities are very high
# For those with breast cancer they are likely to have any probability.
...
Record AUC and Brier score of the model on the original dataset. This will be needed in the next step.
```{r}
...
for (i in 1:500){
  ...
  bootsmpl <- bc[index,] #@
  boot.m <- glm(my.formula, family="binomial", data=bootsmpl) #@
  probs.m <- predict(boot.m, type="response") #@
  probs.orig <- predict(boot.m, newdata = bc, type="response") #@

  pred.m <- ROCR::prediction(probs.m, bootsmpl$Class)
  auc.m <- performance(pred.m, "auc") #@
  a.m <- slot(auc.m, "y.values")[[1]] #@
  AUCS.m <<- c(AUCS.m, a.m) #@
  brier.m <- mean((probs.m - bootsmpl$Class)^2) #@
  BRIERS.m <<- c(BRIERS.m, brier.m) #@
  pred.orig <- ROCR::prediction(probs.orig, bc$Class)
  auc.orig <- performance(pred.orig, "auc") #@
  a.orig <- slot(auc.orig, "y.values")[[1]] #@
  brier.orig <- mean((probs.orig - (bc$Class))^2) #@
  AUCS.orig <<- c(AUCS.orig, a.orig) #@
  BRIERS.orig <<- c(BRIERS.orig, brier.orig) #@
}

mean(AUCS.m)
CIauc <- quantile(AUCS.m-AUCS.orig, probs=c(0.025, 0.975)) #@
CIbrier <- quantile(BRIERS.m - BRIERS.orig, probs=c(0.025, 0.975)) #@


AUC.ORIGINAL - mean(AUCS.m - AUCS.orig) #@
quantile(AUC.ORIGINAL - (AUCS.m - AUCS.orig), probs=c(0.025, 0.975)) #@

BRIER.ORIGINAL - mean(BRIERS.m - BRIERS.orig) #@
...
=> implement the variable selection strategy with stepAIC (use the direction "bakward") on the original model and inside the bootstrap sample. What is the optimism-corrected AUC of this strategy? What is the final model?

=> OPEN part (40% of the grade): implement a strategy based on a non parametric model of your choice like a decision tree or random forests. Compare, in terms of the AUCs and Brier scores, this new strategy with the strategy based on logistic regression with stepAIC. Show clearly if and how you control for complexity in the new strategy and how do you tell whether the differences in performance are statistically significant? You need to find out answers to these questions yourself.