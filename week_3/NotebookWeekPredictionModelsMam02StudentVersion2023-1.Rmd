...
# train
rf_model <- randomForest(Class ~ ., data=train_rf, ntree=50, mtry=3, nodesize=5) 
# ntree controls the number of trees, we chose 50 to limit complexity and computation
# mtry the number of variables per tree. This is recommended to be set at n(params)/3 == 3
# nodesize sets the size of the tree 5 is chosen to keep the trees small enough
...
test_rf$Class <- as.numeric(as.character(test_rf$Class))
brier2 <- mean((rf_predictions - test_rf$Class)^2)
brier2

# brier of the random forrest model [1] 0.03695941
# brier of the other model [1] 0.02104547

# convert class to numeric as well
bc$Class <- as.factor(as.character(bc$Class))

AUCS.rf <<- vector(mode="integer", length=0) # Vector to save the AUCs of bootsrtap models tested in their own boostrap sample.
BRIERS.rf <<- vector(mode="integer", length=0)  # Vector for the Brier scores on the bootstrap samples
AUCS.random <<- vector(mode="integer", length=0)  # Vector for the AUCs of bootsrtrap models tested on the original dataset
BRIERS.random <<- vector(mode="integer", length=0)  # Vector for the Briers of bootsrtrap models tested on the original dataset
for (i in 1:500){
  index <- sample(1:nrow(bc), replace=T) #@ Generate a random sample of indicies from the columns of bc
  bootsmpl <- bc[index,] #@ create a dataset with the actual rows derived form the indicies 
  boot.m <- randomForest(Class ~ ., data=bootsmpl, ntree=50, mtry=3, nodesize=5)  #@ Fits the random forrest model on the sample test
  probs.m <- predict(boot.m, type = "prob")[,2] #@ set response type to get probabilities, predict using the fitted linear predictors (sample)
  probs.orig <- predict(boot.m, newdata = bc, type = "prob")[,2] #@ predict on the whole dataset

  bootsmpl$Class <- as.numeric(as.character(bootsmpl$Class))
  pred.m <- ROCR::prediction(probs.m, bootsmpl$Class) #@ predict again using ROCR 
  auc.m <- performance(pred.m, "auc") #@ receive AUC measure form the pred.m object
  a.m <- slot(auc.m, "y.values")[[1]] #@ retreive AUC form object
  AUCS.rf <<- c(AUCS.rf, a.m) #@ save the AUC object
  brier.rf <- mean((probs.m - bootsmpl$Class)^2) #@ compute Brier score
  BRIERS.rf <<- c(BRIERS.rf, brier.m) #@ save Brier score
  bc$Class <- as.numeric(as.character(bc$Class))
  pred.orig <- ROCR::prediction(probs.orig, bc$Class) #@ perform prediction unsing ROCR on the orig dataset
  auc.orig <- performance(pred.orig, "auc") #@ ask for the auc object 
  a.orig <- slot(auc.orig, "y.values")[[1]] #@ retrive the auc object
  brier.orig <- mean((probs.orig - (bc$Class))^2) #@ compute brier score
  AUCS.random <<- c(AUCS.random, a.orig) #@ save AUC score
  BRIERS.random <<- c(BRIERS.random, brier.orig) #@ save briers score

  # auc_diff[i] <- rf_auc - lr_auc
}

ci <- quantile(auc_diff, probs = c(0.025, 0.975))

mean(AUCS.rf)
# [1] 0.9971638
CIauc <- quantile(AUCS.rf-AUCS.random, probs=c(0.025, 0.975)) #@ create quantiles for confidence intervals fpr the aucs
CIbrier <- quantile(BRIERS.rf - BRIERS.random, probs=c(0.025, 0.975)) #@ create quantitls for confidence intervals of the brier scores
# [1] 0.9970672

AUC.ORIGINAL - mean(AUCS.rf - AUCS.random) #@ get difference of the aucs on the original dataset and averavge (mean) Auc
quantile(AUC.ORIGINAL - (AUCS.rf - AUCS.random), probs=c(0.025, 0.975)) #@ compute confidence interval for AUCs of the original dataset
#   2.5%     97.5% 
# 0.9931169 1.0012109 

BRIER.ORIGINAL - mean(BRIERS.rf - BRIERS.random) #@ get difference of the briar scores of the orifinal dataset and the mean of the brierscores
# [1] 0.003930203
quantile(BRIER.ORIGINAL - (BRIERS.rf - BRIERS.random), probs=c(0.025, 0.975)) #@ get quantilees of the briar scores
#        2.5%        97.5% 
# 0.0003663336 0.0078287980 
